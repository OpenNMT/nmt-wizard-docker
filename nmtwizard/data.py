...
def merge_files_in_directory(input_dir, output_dir, src_suffix, tgt_suffix):
    ...
    src_files = sorted(
        os.path.join(input_dir, f) for f in files if f.endswith(src_suffix)
    )
    if tgt_suffix:
        tgt_files = sorted(
            os.path.join(input_dir, f) for f in files if f.endswith(tgt_suffix)
        )
    align_files = sorted(
        os.path.join(input_dir, f) for f in files if f.endswith("align")
    )
    weight_files = sorted(
        os.path.join(input_dir, f) for f in files if f.endswith("weights")
    )
