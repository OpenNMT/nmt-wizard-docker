    ...
    def replace_tokens(self, start_idx, tok_num, new_tokens=None, part=0):
        ...
        if cur_tokens is not None:
            ...
            if start_idx > cur_length:
                raise IndexError(
                    "Start index is too big for replacement.\tTOKENS: {}\tSTART IDX: {}\tNEW TOKENS: {}".format(
                        cur_tokens, start_idx, new_tokens
                    )
                )
            ...
            if end_idx > cur_length:
                raise IndexError(
                    "Too many tokens to delete.\tTOKENS: {}\tSTART IDX: {}\tNEW TOKENS: {}".format(
                        cur_tokens, start_idx, new_tokens
                    )
                )
