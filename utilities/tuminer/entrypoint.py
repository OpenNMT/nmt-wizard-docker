...
def setCUDA_VISIBLE_DEVICES(gpuid):
    if gpuid == 0:
        ...
        logger.debug(
            " - set CUDA_VISIBLE_DEVICES= %s" % os.environ["CUDA_VISIBLE_DEVICES"]
        )
...
def bpe(bpecodes, inputF, outputF, verbose):
    BPEfastApply(inputF, outputF, bpecodes, verbose=verbose, over_write=False)
...
def loadEncoder(
    encoderF, buffer_size, max_tokens, max_sentences=None, cpu=False, stable=False
):
...
def openOutputF(filename, encoding):
    if filename.endswith(".gz"):
        return gzip.open(
            filename, mode="wt", encoding=encoding, errors="surrogateescape"
        )
    return open(filename, mode="w", encoding=encoding, errors="surrogateescape")


def scoreBitext(
    src_inds, trg_inds, x, y, x2y_mean, y2x_mean, outputF, encoding, margin
):
...
def mineBitext(
    ...
    logger.info(
        " - writing mined output to {:s}, {:s}, {:s}".format(
...
class TuminerUtility(Utility):
    ...
    def declare_arguments(self, parser):
        ...
        parser_miner.add_argument(
            "--tumode", required=True, choices=["score", "mine"], help="Tuminer mode"
        )
        parser_miner.add_argument(
            "--srclang",
            required=False,
        ...
        parser_miner.add_argument(
            "--srcfile", required=True, help="Source language file."
        )
        parser_miner.add_argument(
            "--tgtlang",
            required=False,
        ...
        parser_miner.add_argument(
            "--tgtfile", required=True, help="Target language file."
        )
        parser_miner.add_argument("--output", required=True, help="Output file.")
        parser_miner.add_argument(
            "--encoding",
            required=False,
            default="utf-8",
    ...
    def exec_function(self, args):
        ...
        outputF_local = os.path.join(
            self._data_dir, self._storage.split(args.output)[-1]
        )

        if args.bpecodes is not None:
            bpeCodesF_local = os.path.join(
                self._data_dir, self._storage.split(args.bpecodes)[-1]
            )
            self._storage.get_file(args.bpecodes, bpeCodesF_local)
        if args.encoder is not None:
            encoderF_local = os.path.join(
                self._data_dir, self._storage.split(args.encoder)[-1]
            )
        ...
        logger.info(
            "srclang: %s, srcfile: %s (%s)" % (args.srclang, args.srcfile, srcF_local)
        )
        logger.info(
            "tgtlang: %s, tgtfile: %s (%s)" % (args.tgtlang, args.tgtfile, tgtF_local)
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if isinstance(args.gpuid, list):
                ...
                srcP = mp.Process(
                    ...
                    args=(
                        ...
                        bpeCodesF_local,
                        encoderF_local,
                        args.encoderbuffersize,
                        args.encodermaxtokens,
                        args.verbose,
                        args.gpuid[0],
                    ),
                )
            ...
            src_inds, src_sents, x = _loadTextAndEmb(
                srcF_local, args.encoding, srcEmbF, args.encoderdim, unify, args.verbose
            )
            trg_inds, trg_sents, y = _loadTextAndEmb(
                tgtF_local, args.encoding, tgtEmbF, args.encoderdim, unify, args.verbose
            )

            # calculate knn in both directions
            if retrieval != "bwd":
                logger.info(
                    " - perform {:d}-nn source against target".format(neighborhood)
                )
            ...
            if retrieval != "fwd":
                logger.info(
                    " - perform {:d}-nn target against source".format(neighborhood)
                )
            ...
            if args.tumode == "score":
                ...
                statCnt, statMin, statMax, statAvg, statStddev = getScoreDist(
                    outputF_local
                )
                ...
                foutSrc, foutSrc_remote = (
                    outputF_local + "." + args.srclang + src_suffix,
                    args.output + "." + args.srclang + src_suffix,
                )
                if srcF_local.endswith(".gz"):
                    foutSrc = foutSrc + ".gz"
                    foutSrc_remote = foutSrc_remote + ".gz"

                foutTgt, foutTgt_remote = (
                    outputF_local + "." + args.tgtlang + tgt_suffix,
                    args.output + "." + args.tgtlang + tgt_suffix,
                )
                if tgtF_local.endswith(".gz"):
                    foutTgt = foutTgt + ".gz"
                    foutTgt_remote = foutTgt_remote + ".gz"

                foutScore, foutScore_remote = (
                    outputF_local + ".tuminer-score",
                    args.output + ".tuminer-score",
                )
            ...
            logger.info(
                "Score statistics -- CNT: {:d}, MIN: {:f}, MAX: {:f}, AVG: {:f}, STDDEV: {:f}".format(
                    statCnt, statMin, statMax, statAvg, statStddev
                )
            )
